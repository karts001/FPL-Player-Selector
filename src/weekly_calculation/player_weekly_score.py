...
def calculate_player_score():
    full_path = _get_data_from_csv_file()
    df_csv = _convert_csv_data_to_data_frame(full_path)
    df_fpl_api = _get_data_from_fpl_api()
    df_combined = _concatenate_the_two_data_frames(df_csv, df_fpl_api)
    df_probabilities = _add_gameweek_odds_as_new_column_of_the_dataframe(df_combined)
...
def _get_data_from_csv_file():
    # Get data from csv file
    full_path = fp.get_filtered_players_csv_path()
    return full_path

def _convert_csv_data_to_data_frame(full_path):
...
def _concatenate_the_two_data_frames(df_fpl_and_roi, df_fpl):
...
def _calcualte_fpl_weekly_score_and_add_to_data_frame(df_fpl):
    ...
    df_fpl.loc[df_fpl["chance_of_playing_this_round"] == 0.0, "fpl_weekly_score"] = 0
